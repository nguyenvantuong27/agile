...
import { useGetBranchByIdQuery } from '~/services/branches/branches.services';

const Header = () => {
  const [isUserMenuOpen, setIsUserMenuOpen] = useState(false);
  const auth = useAppSelector((state: RootState) => state.auth.currentUser);

  const { data: branch, isLoading: isBranchFetch } = useGetBranchByIdQuery(
    auth.branch_id,
  );
    ...
    <div className="bg-black text-white p-4 fixed w-full top-0 flex justify-between items-center shadow-lg z-[999]">
      ...
      <div className="flex items-center space-x-4">
        ...
        <div className="relative">
          <button className="flex items-center bg-white text-black px-4 py-3 rounded-full font-bold shadow border border-black">
            {isBranchFetch ? <p>đang tải...</p> : branch?.data.name}
            ...
            <div className="absolute right-0 mt-2 w-[180px] bg-white rounded-lg shadow-lg border border-gray-300">
              ...
              <ul className="py-2">
                <li className="px-4 py-2 hover:bg-gray-100 cursor-pointer flex items-center">
                  ...
                  <Link to={`/artist/profile/${auth._id}`}>
