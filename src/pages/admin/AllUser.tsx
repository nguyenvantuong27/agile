...
const AllUser: React.FC = () => {
  ...
  const { data, error, isLoading, refetch } =
    useGetUsersWithArtistOrEmployeeRoleQuery({ limit, page });
  ...
  const [loadingStates, setLoadingStates] = useState<{
    [key: string]: { patch?: boolean; block?: boolean; unblock?: boolean };
  }>({});
  ...
  const onSubmit = async (data: IUser) => {
    ...
    try {
      ...
      Toastify('Cập nhật thông tin nhân viên thành công', 200);
  ...
  return (
    <div className="p-6">
      <h1 className="text-2xl font-bold mb-6 text-gray-800">
        Danh sách nhân viên ({filteredData.length})
      </h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredData.length > 0 ? (
          ...
          <div className="col-span-full text-center text-gray-500">
            Không có nhân viên nào đã xác thực
          </div>
        )}
      </div>
      ...
      {isModalOpen && selectedUser && (
        <Modal open={isModalOpen}>
          <Modal.Header>
            <h3 className="text-lg font-bold">Cập nhật thông tin nhân viên</h3>
          </Modal.Header>
          <Modal.Body>
